...
// TODO: figure out why this breaks as u64
typedef u64 time_t;

#define JS_FN(name)                                                            \
  __attribute__((import_module("env")))                                        \
  __attribute__((import_name(name))) extern

#define EXPORT(name) __attribute__((export_name(name)))

#define ARRAY_LEN(x) (sizeof(x) / sizeof(x[0]))

#define UNIQUE_VAR_NAME(prefix) prefix##_##__LINE__

#define STR_HELPER(x) #x
#define STR(x) STR_HELPER(x)
#define ASSERT_STACKTRACE(x)                                                   \
  __FILE__ "(" STR(__LINE__) "): Assertion failed for '" #x "'"
...
EXPORT("walloc")
void *walloc(size_t size) { return malloc(size); }
...
void srand(unsigned seed) { rand_state = pcg32_init(seed); }
